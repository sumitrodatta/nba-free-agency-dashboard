...
for_similar=read_csv("Data/Train Set.csv")

row_numbers=for_similar %>% select(seas_id) %>% mutate(row=row_number())

pre_sim_matrix=for_similar %>% mutate(type=case_when(type=="UFA"~0,type=="RFA"~1)) %>% 
...
similarity_scores=as_tibble(as.matrix(dist(scale(pre_sim_matrix)))) %>% 
  ...
  mutate(self_compare=(rowid==to_compare),similarity=round(1-similarity/ncol(pre_sim_matrix),digits = 4)) %>% 
  filter(!self_compare) %>%
